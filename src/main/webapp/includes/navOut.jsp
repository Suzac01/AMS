<nav class="navbar navbar-expand-lg navbar-light bg-light" style="text-align: center;">
    <div style="text-align: center;" class="container" id="next-con">
        <img class="Logo" src="assets/img/logo.png" alt="No logo" width="60px">
        <div class="content">
            <div class="attendance">
                Attendance
            </div>
            <div class="management-system">
                Management System
            </div>
        </div>
        <style>
            /* Define styles for the "Attendance" text */
            .attendance {
                font-size: 32px;
                font-weight: 550;
                text-align: left;
            }
            /* Define styles for the "Management System" text */
            .management-system {
                font-size: 18px;
                font-weight: normal;
                color: rgb(36, 36, 36);
            }
        </style>
        <button class="navbar-toggler" type="button" data-bs-toggle="collapse"
                data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent"
                aria-expanded="false" aria-label="Toggle navigation"></button>
        <div class="collapse navbar-collapse" style="justify-content: end" id="navbarSupportedContent">
<%--            <ul class="navbar-nav ms-auto mb-2 mb-lg-0">--%>
               <button class="button" style="background-color: black; border-radius: 4px; height: 45px; width: 100px" > <a  href="Login.html" style="text-decoration: none; color: whitesmoke">Log out</a> </button>
<%--            </ul>--%>
        </div>
    </div>
</nav>